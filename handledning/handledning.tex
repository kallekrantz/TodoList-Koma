...
\fbox{
  \parbox{\textwidth}{
	Uppgift: Gör en lista i det grafiska gränssnittet.
...
\fbox{
  \parbox{\textwidth}{
	Uppgift: Implentera onListItemClick och skriv ut någon text genom LogCat för att se att interaktionen med listan funkar korrekt. 
	}
}
...
\fbox{
  \parbox{\textwidth}{
	Uppgift: Använd dig av \textit{AlertDialog} och dess tillhörande \textit{Builder} för att skapa en dialogruta som visas när man trycker på en post i listan.
	}
}
...
Lägg nu till en knapp(\textit{Form Widgets -> Button}) och ett textfält (\textfit{Text Fields -> Plain Text}) så de hamnar i din \textit{LinearLayout(horizontal)}. Ge dem ett lämpligt \textit{id} i \textit{Properties}, tex "add" för knappen och "name" för textfältet. Kom ihåg att det måste vara på formen \textit{@+id/ditt namn här}.\\
...
Skapa först knappen genom att skriva \input{code/button}
